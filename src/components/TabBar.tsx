...
export default function TabBar({
  ...
  return (
    <div className={`tabbar${isMac ? " platform-mac" : " platform-windows"}`}>
      ...
      <div className="tab-strip" role="tablist" aria-label="Open repositories">
        {tabs.map((t, i) => (
          <div
            key={t.path}
            ref={(el) => {
            ...
            tabIndex={t.path === activePath ? 0 : -1}
            className={
...
function WindowControls({ platform }: { platform: "mac" | "windows" }) {
  ...
  const onMaximize = async (e: React.MouseEvent) => {
    ...
    if (e.altKey) {
      ...
      void appWindow.setFullscreen(!isFull);
    }
  };
    ...
    <div className={`window-controls ${platform}`}>
      ...
      >
        {platform === "windows" ? (
          ...
          <svg className="mac-glyph" viewBox="0 0 12 12" aria-hidden="true">
            <path d="M3.5 3.5l5 5M8.5 3.5l-5 5" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" />
          </svg>
        )}
      </button>
      <button
      ...
      <button
        ...
        onClick={onMaximize}
        title={platform === "mac" ? (altDown ? "Zoom" : "Enter Full Screen") : "Maximize"}
        aria-label={platform === "mac" ? (altDown ? "Zoom" : "Enter Full Screen") : "Maximize"}
      >
        {platform === "windows" ? (
          <svg viewBox="0 0 10 10" aria-hidden="true">
          ...
          <svg className="mac-glyph" viewBox="0 0 12 12" aria-hidden="true">
            <path d="M6 2.5v7M2.5 6h7" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" />
          </svg>
        ) : (
          <svg className="mac-glyph" viewBox="0 0 12 12" aria-hidden="true">
            <path d="M5.3 3.5h3.2v3.2z M6.7 8.5h-3.2v-3.2z" fill="currentColor" stroke="none" />
          </svg>
        )}
      </button>
    </div>
